...
sub xyxy_to_n {
  ...
  my $n;
  foreach my $n1 ($self->xy_to_n_list($x1,$y1)) {
    foreach my $n2 ($self->xy_to_n_list($x2,$y2)) {
      if ($n2 == $n1 + $arms) {
        if (! defined $n || $n1 < $n) {
          $n = $n1;
        }
...
Return <$n> which goes from C<$x1,$y1> to C<$x2,$y2>.  <$n> is at
C<$x1,$y1> and C<$n+1> is at C<$x2,$y2>.  For a multi-arm path C<$n+$arms>
so a step along the same arm.  If there's no such C<$n> then return
C<undef>.

For C<xyxy_to_n_either()> the <$n> can be either direction, so C<$x1,$y1>
to C<$x2,$y2> or the other way C<$x2,$y2> to C<$x1,$y1>.
...
A subclass can implement is directly if they can be done more efficiently.

    xy_is_visited()       defined(xy_to_n($x,$y))
    xyxy_to_n()           calls xy_to_n_list()
    xyxy_to_n_either()    calls xy_to_n_list()
